...
contract VotingSystem {
    ...
    modifier onlyCommission() {
        require(
            ...
            "Chỉ Ủy ban bầu cử mới có quyền thực hiện thao tác này"
        );
        _;
    }
    ...
    function createElectionProposal(
        ...
        for (uint i = 0; i < _candidateIds.length; i++) {
            ...
            require(
                ...
                "Ứng viên đã thuộc cuộc bầu cử khác"
            );
        }
        ...
        // Đánh dấu ứng viên đang trong đề xuất
        for (uint i = 0; i < _candidateIds.length; i++) {
            candidates[_candidateIds[i]].isBelong = proposalCount + 1000000; // Đánh dấu thuộc đề xuất (thêm 1000000 để phân biệt với ID cuộc bầu cử)
        }
    ...
    function approveProposal(uint _proposalId) public onlyCommission {
        ...
        // Cập nhật trạng thái cho các ứng viên
    ...
    function rejectProposal(
        ...
        // Giải phóng các ứng viên của đề xuất bị từ chối
        for (uint i = 0; i < proposal.candidateIds.length; i++) {
            uint candidateId = proposal.candidateIds[i];
    ...
    function hasVoted(uint _electionId) public view returns (bool) {
        require(
            _electionId > 0 && _electionId <= countElections,
            "ID cuộc bầu cử không hợp lệ"
        );
    ...
    function vote(uint _electionId, uint _candidateId) public {
        ...
        require(
            ...
            "Bạn đã bỏ phiếu trong cuộc bầu cử này"
        );
    ...
    function endElection(uint _electionId) public onlyAdmin {
        ...
        // Giải phóng các ứng viên của cuộc bầu cử đã kết thúc
    ...
    function getPendingProposals() public view returns (uint[] memory) {
        ...
        for (uint i = 1; i <= proposalCount; i++) {
            if (
                !proposals[i].isApproved &&
                bytes(proposals[i].rejectionReason).length == 0
            ) {
                pendingCount++;
            }
        }
